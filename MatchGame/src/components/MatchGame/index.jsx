...
export default class MatchGame extends Component {
  ...
  startTimer = () => {
    this.setState((prevState) => {
      if (prevState.isPlaying && prevState.timer > 0) {
  ...
  render() {
    const { score, timer, activeTabId, matchImageId, isPlaying } = this.state;

    const filteredList = imagesList.filter(
      (item) => activeTabId === item.category
    );

    const matchImage = imagesList.find((item) => item.id === matchImageId);
        ...
        <div className="game-cnt">
          {isPlaying ? (
            <>
              ...
              <ul className="thumbnail-items-cnt">
                {filteredList.map((thumbnailItem) => (
                  <ThumbnailItem
